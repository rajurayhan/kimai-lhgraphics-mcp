...
import { KimaiClient } from "./kimai-client.js";
...
export function createKimaiMcpServer(client: KimaiClient): Server {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
      ...
      {
        name: "kimai_payroll_ping",
        description:
          "LHG Payroll: health check (GET /api/payroll/ping). Requires LhgPayrollBundle.",
        inputSchema: { type: "object", properties: {} },
      },
      {
        name: "kimai_payroll_statuses",
        description:
          "LHG Payroll: approval status codes 1–6 (GET /api/payroll/statuses).",
        inputSchema: { type: "object", properties: {} },
      },
      {
        name: "kimai_payroll_period",
        description:
          "LHG Payroll: biweekly period start/end for a reference date (GET /api/payroll/period).",
        inputSchema: {
          type: "object",
          properties: {
            date: {
      ...
      {
        name: "kimai_payroll_biweekly",
        description:
          "LHG Payroll: timesheets, totals, project breakdown, and current approval for a biweekly period (GET /api/payroll/biweekly).",
        inputSchema: {
          type: "object",
          properties: {
            date: {
            ...
            user_id: {
              type: "string",
              description:
                "Target user ID; omit for self. Team lead / super admin only.",
            },
          },
        },
      },
      {
        name: "kimai_payroll_queues",
        description:
          "LHG Payroll: submitted, approved, and not-submitted queues for the biweekly period (GET /api/payroll/queues).",
        inputSchema: {
          type: "object",
          properties: {
            date: {
      ...
      {
        name: "kimai_payroll_users",
        description:
          "LHG Payroll: users with non-zero hourly rates (GET /api/payroll/users). Requires view_user or api_payroll_view_all.",
        inputSchema: { type: "object", properties: {} },
      },
      {
        name: "kimai_payroll_users_accessible",
        description:
          "LHG Payroll: users the caller may view payroll for — self, team, or all (GET /api/payroll/users/accessible).",
        inputSchema: { type: "object", properties: {} },
      },
      {
        name: "kimai_payroll_approvals_list",
        description:
          "LHG Payroll: list approvals visible to the current user (GET /api/payroll/approvals).",
        inputSchema: {
          type: "object",
          properties: {
            status: {
              type: "string",
              description: "Filter by StatusEnum value (1–6)",
            },
            start_date: {
              type: "string",
              description: "Filter by period start YYYY-MM-DD",
            },
          },
        },
      },
      {
        name: "kimai_payroll_approval_get",
        description:
          "LHG Payroll: approval detail with timesheets and history (GET /api/payroll/approvals/{id}).",
        inputSchema: {
          type: "object",
          properties: { id: { type: "integer", description: "Approval ID" } },
          required: ["id"],
        },
      },
      {
        name: "kimai_payroll_submit",
        description:
          "LHG Payroll: submit a biweekly period for approval (POST /api/payroll/approvals).",
        inputSchema: {
          type: "object",
          properties: {
            userId: { type: "integer", description: "Kimai user ID" },
            startDate: {
              type: "string",
      ...
      {
        name: "kimai_payroll_update_status",
        description:
          "LHG Payroll: team lead or finance approve/reject (POST /api/payroll/approvals/{id}/status). Status 2=TL approve, 3=TL reject, 4=finance approve (include commission/adjustment/deduction/netPayable/paymentMethod), 5=finance reject.",
        inputSchema: {
          type: "object",
          properties: {
            ...
            status: {
              type: "integer",
              description: "StatusEnum value 1–6",
            },
            message: { type: "string" },
            ...
            paymentMethod: {
              type: "string",
              description:
                "Payoneer, Paypal, Patriot Software, Wise, Upwork, or Zelle",
            },
          },
          required: ["id", "status"],
        },
      ...
      {
        name: "kimai_payroll_resubmit",
        description:
          "LHG Payroll: re-submit a rejected approval (POST /api/payroll/approvals/{id}/resubmit).",
        inputSchema: {
          type: "object",
          properties: { id: { type: "integer", description: "Approval ID" } },
          required: ["id"],
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (name === "kimai_payroll_ping") {
        const r = await client.request("GET", "/api/payroll/ping");
        return {
          content: [
            {
              type: "text",
      ...
      if (name === "kimai_payroll_statuses") {
        const r = await client.request("GET", "/api/payroll/statuses");
        return {
          content: [
            {
              type: "text",
      ...
      if (name === "kimai_payroll_period") {
        const q = asStringRecord(args);
        const r = await client.request("GET", "/api/payroll/period", {
          query: q,
        });
        return {
          content: [
      ...
      if (name === "kimai_payroll_biweekly") {
        const q = asStringRecord(args);
        const r = await client.request("GET", "/api/payroll/biweekly", {
          query: q,
        });
        return {
          content: [
      ...
      if (name === "kimai_payroll_queues") {
        const q = asStringRecord(args);
        const r = await client.request("GET", "/api/payroll/queues", {
          query: q,
        });
        return {
          content: [
      ...
      if (name === "kimai_payroll_users") {
        const r = await client.request("GET", "/api/payroll/users");
        return {
          content: [
            {
              type: "text",
      ...
      if (name === "kimai_payroll_users_accessible") {
        const r = await client.request("GET", "/api/payroll/users/accessible");
        return {
          content: [
            {
              type: "text",
      ...
      if (name === "kimai_payroll_approvals_list") {
        const q = asStringRecord(args);
        const r = await client.request("GET", "/api/payroll/approvals", {
          query: q,
        });
        return {
          content: [
      ...
      if (name === "kimai_payroll_approval_get") {
        const id = Number(args.id);
        const r = await client.request("GET", `/api/payroll/approvals/${id}`);
        return {
          content: [
            {
              type: "text",
      ...
      if (name === "kimai_payroll_submit") {
        const r = await client.request("POST", "/api/payroll/approvals", {
          body: {
      ...
      if (name === "kimai_payroll_update_status") {
        const id = Number(args.id);
        const body: Record<string, unknown> = {
          status: Number(args.status),
        };
        if (args.message !== undefined) body.message = args.message;
        if (args.commission !== undefined) body.commission = args.commission;
        if (args.adjustment !== undefined) body.adjustment = args.adjustment;
        if (args.deduction !== undefined) body.deduction = args.deduction;
        if (args.netPayable !== undefined) body.netPayable = args.netPayable;
        if (args.paymentMethod !== undefined) {
          body.paymentMethod = args.paymentMethod;
        }
        const r = await client.request(
          "POST",
          `/api/payroll/approvals/${id}/status`,
          { body },
        );
        return {
          content: [
      ...
      if (name === "kimai_payroll_resubmit") {
        const id = Number(args.id);
        const r = await client.request(
          "POST",
          `/api/payroll/approvals/${id}/resubmit`,
        );
        return {
          content: [
            {
